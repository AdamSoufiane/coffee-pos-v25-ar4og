...
public class InfrastructureMongoProductRepositoryImpl implements InfrastructureProductRepositoryPort {
    ...
    public Optional<DomainProductEntity> findById(String id) {
        try {
            return Optional.ofNullable(mongoTemplate.findById(id, DomainProductEntity.class));
        } catch (Exception e) {
    ...
    public boolean existsByName(String name) {
        try {
            ...
            return mongoTemplate.exists(query, DomainProductEntity.class);
        } catch (Exception e) {
            logger.error("Error checking existence of product by name: {}", name, e);
            throw new RuntimeException("Error checking existence of product by name", e);
        }
    }
    ...
    private void validateProductData(DomainProductEntity product) {
        ...
        if (product.getCategoryId() == null || product.getCategoryId().isEmpty()) {
            throw new IllegalArgumentException("Product categoryId must not be empty");
        }
